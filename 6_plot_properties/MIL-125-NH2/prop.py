...
energy_files = glob.glob('../4_nacs/MIL-125-NH2/res-mixed-basis/Hvib_sd*re*')
...
plt.title('MIL-125-NH$_2$ - Mixed basis', fontsize=10)
...
# Plot NAC map
nac_files = glob.glob('../4_nacs/MIL-125-NH2/res-mixed-basis/Hvib_sd*im*')
...
plt.title('MIL-125-NH$_2$ - Mixed basis', fontsize=10)
plt.tight_layout()
plt.savefig('mof_mixed_basis_nac_map.jpg', dpi=600)
np.savetxt('nac_ave_sd', nac_ave)

# In[10]:

nac_files = glob.glob('../4_nacs/MIL-125-NH2/res-ks/Hvib_ks*im*')
for c, nac_file in enumerate(nac_files):
    nac_mat = sp.load_npz(nac_file).todense().real
    if c==0:
        nac_ave = np.zeros(nac_mat.shape)
...
plt.ylabel('State index', fontsize=10)
plt.colorbar().ax.set_title('meV', fontsize=8)
plt.title('MIL-125-NH$_2$ - KS', fontsize=10)
plt.tight_layout()
plt.savefig('mof_ks_basis_nac_map.jpg', dpi=600)
...
#get_ipython().run_line_magic('matplotlib', 'notebook')
plt.figure(figsize=(3.21, 2.41), dpi=600, edgecolor='black', frameon=True)
path_to_all_pdos = '../3_overlaps/MIL-125-NH2/all_pdosfiles'
atoms = ['C','H','N','O','Ti']
...
for c1,i in enumerate([1,2,3,4,5]):
...
plt.title('MIL-125-NH$_2$, 300 K', fontsize=10)
plt.tight_layout()
#plt.savefig('../C3N4_pdos.jpg', dpi=600)
plt.savefig('mof_pdos_1.jpg', dpi=600)
